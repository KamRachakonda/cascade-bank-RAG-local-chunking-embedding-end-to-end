...
Usage:
    ...
    require_groq_key()  # raises RuntimeError with a clear message if unset
"""
...
class Config(BaseSettings):
    ...
    # Secret, read from the GROQ_API_KEY env var (via .env or the shell).
...
def require_groq_key() -> str:
    """
    Return the Groq API key, or raise a clear RuntimeError if it is missing.
    Use this right before making a Groq API call.
    """
    cfg = get_config()
    if not cfg.groq_api_key:
        raise RuntimeError(
            "GROQ_API_KEY is not set. Copy .env.example to .env and set "
            "GROQ_API_KEY to a key from https://console.groq.com/keys, "
