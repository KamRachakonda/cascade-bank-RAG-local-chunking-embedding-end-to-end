...
`src.rerank.cross_encoder_rerank.search_and_rerank`) and either:
    - synthesizes a grounded, cited answer via Groq (`synthesize_answer`), or
...
Import-safety note: this module does not require GROQ_API_KEY at import
...
from src.generate.groq_client import chat_completion
...
def synthesize_answer(query: str, chunks: list[dict], stream: bool = False) -> str | Iterable:
    """
    Build a context block from `chunks`, prompt the Groq model to answer
...
def answer_only_mode(query: str, chunks: list[dict]) -> str:
    ...
    and a note that no LLM call was made. Useful when GROQ_API_KEY is
...
def main() -> None:
    ...
    parser.add_argument("--k", type=int, default=None,
                        help="Semantic search top_k (default: cfg.search.top_k)")
    parser.add_argument("--n", type=int, default=None,
                        help="Rerank top_n (default: cfg.rerank.top_n)")
    parser.add_argument("--doc-type", default=None,
                        help="Restrict to a specific doc_type metadata value")
    args = parser.parse_args()

    from src.rerank.cross_encoder_rerank import search_and_rerank

    result = search_and_rerank(
        args.query, top_k=args.k, top_n=args.n, doc_type=args.doc_type)
    ...
    if not os.environ.get("GROQ_API_KEY"):
        print(
            "(No GROQ_API_KEY set -- falling back to answer-only mode, no LLM call made.)")
        print()
